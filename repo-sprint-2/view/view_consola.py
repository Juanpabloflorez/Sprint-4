...
class App(tk.Tk):
    def __init__(self):
        ...
        for F in (Mainview, Loginview, Crearview, Homeview, Agregarview, Quitarview, Editarview, Marcarcompletasview, Vertareasview, Beesview):
            nombre = F.__name__
...
class Homeview(tk.Frame):
    def __init__(self, parent, controller):
        ...
        label = tk.Label(self, text="Bienvenido", font=("Arial", 16), bg="Lightblue")
        label.pack(pady=0, fill="x")

        self.imagen1 = tk.PhotoImage(file = "repo-sprint-2/view/logo.png")
        self.imagen1 = self.imagen1.subsample(12, 12)
        self.logo = tk.Label(self, image=self.imagen1)
        ...
        verbees = tk.Button(self, text="🐝 Ver mis Bees 🐝", bg="lightgray", width=20, command=lambda: controller.mostrar_frame("Beesview"), background="Yellow")
        verbees.place(x=400, y=60)
...
class Beesview(tk.Frame):
    def __init__(self, parent, controller):
        super().__init__(parent, bg="Lightyellow")

        obtener = tk.Button(self, text="Cargar mis Bees", bg="yellow", command=lambda: (self.obtenerbeeslabel()))
        obtener.place(x=30, y=50)

        message = tk.Label(self, text="Para obtener mas Bees, completa más tareas para que las Bees vean que eres confiable", bg="yellow")
        message.place(x=140, y=50)

        self.imagen1 = tk.PhotoImage(file = "repo-sprint-2/view/colmena.png")
        self.imagen1 = self.imagen1.subsample(7, 7)
        self.logo = tk.Label(self, image=self.imagen1)
        self.logo.place(x=30, y=100)
    
    def obtenerbeeslabel(self):
        beesget = repo_tarea.TareaRepository.obtenerbees(self, currentuser)
        bees = []
        for i in range(beesget):
            bees.append("🐝")
        name = tk.Label(self, text=bees, bg="yellow")
        name.place(x=100, y=100)

        name2 = tk.Label(self, text=f"Tienes {i+1} Bees!", bg="yellow")
        name2.place(x=100, y=140)